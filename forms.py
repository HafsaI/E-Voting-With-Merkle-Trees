...
class Auditing(Form):
    ...
    def audit():
        ...
        if request.method == "POST":
            ...
            audit_data = [option, cand_val]
            valid = run_audit(str(cand_val))
            ...
            if form.validate():
                ...
                if valid[0]:
                    flash(
                        str(valid[1]) + " candidate " + option + " exists.", "success"
                    )

                else:
                    flash("Candidate " + option + " doesn't exist!", "error")

            else:
                flash(
                    "Invalid Password or Invalid selection! Please try again.", "error"
                )
